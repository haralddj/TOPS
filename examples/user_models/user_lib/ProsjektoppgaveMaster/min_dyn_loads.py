...
if __name__ == '__main__':
    ...
    #mport examples.user_models.user_lib.min_sm_ib as model_data
    ...
    # Solver
    sol = dps_sol.ModifiedEulerDAE(ps.state_derivatives, ps.solve_algebraic, 0, x_0, t_end, max_step=4e-2)
    ...
    while t < t_end:
        ...
        # Short circuit
        if 2 <= t: #g_setp refers to the real part of the admittance of the load, b_setp refers to the imaginary part of the admittance
            ps.loads['DynamicLoad'].set_input('g_setp', 0.23, 0)

        if 2 <= t:  #The first number is the new setpoint, as a factor of the base_mva.
                     #The second number is the index of the load in the list of dynamic loads.
            ps.loads['DynamicLoad'].set_input('b_setp', 0.1, 0)

        '''if 2<t and event_flag:
            event_flag = False
        ...
        res['freq'].append(((ps.gen['GEN'].speed(x, v).copy())*50/(2*np.pi))+50.03)
        res['v'].append(v.copy())
    ...
    print(time_normal)
    print(freq_normal)
    timevalues, frequency_data = fc.GetFreqData('/Outage-anonymous (1)/Outage-anonymous.xlsx', 'NO4a:Frequency')
    ...
    ax1.plot(res['time'], avg_freq, label='Simulated frequency')
    ...
    ax2.set_ylabel('Load P [MW]')
    ax2.set_title('Load P over Time')
    ax2.legend()
    ax2.grid(True)
