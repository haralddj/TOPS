...
if __name__ == '__main__':
    ...
    freq_normal = data['Value'][:1000]
    time_normal = []

    for i in range(len(freq_normal)):
        time_normal.append(i/10)

    frequencies, power_spectral_density = periodogram(freq_normal, fs=1)  # fs is the sampling frequency

    # Filter the results to include only frequencies from 0 to 1 Hz
    mask = (frequencies >= 0) & (frequencies <= 1)
    filtered_frequencies = frequencies[mask]
    filtered_psd = power_spectral_density[mask]
    filtered_psd_db = 10 * np.log10(filtered_psd)

    # Plot the Power Spectral Density
    plt.figure()
    plt.semilogy(filtered_frequencies, filtered_psd_db)
    plt.title('Power Spectral Density of the Frequency Data')
    ...
    ax1[1].plot(res['time'], gen_1_freq, label='Simulated frequency')
    ax1[1].plot(time_normal, freq_normal, label='Measured frequency')
    ax1[1].set_xlabel('Time [s]')
    ax1[1].set_ylabel('Frequency [Hz]')
    ax1[1].set_ylim([49.8, 50.2])
    ax1[1].axhline(y=49.9, color='r', linestyle='--')
    ax1[1].axhline(y=50.1, color='r', linestyle='--')
    ax1[1].legend()
    plt.show()
